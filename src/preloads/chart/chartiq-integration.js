...
module.exports = (stx) => {
	...
	const setEmitters = () => {
		FSBL.Clients.DragAndDropClient.setEmitters({
			...
			if (err) {
				Logger.error("Function: setEmitters FSBL.Clients.DragAndDropClient.setEmitters error:", err);
			}  
		});
	}
	// #endregion
	...
	const addReceivers = () => {
		FSBL.Clients.DragAndDropClient.addReceivers({
			...
			if (err) {
				Logger.error("Function: addRecievers FSBL.Clients.DragAndDropClient.addRecievers error:", err);
			}  
		});
	};
	// #endregion
	...
	const subscribeLinker = () => {
		FSBL.Clients.LinkerClient.subscribe("symbol", (symbol, response) => {
			...
			if (err) {
				Logger.error("Function: subscribeLinker FSBL.Clients.LinkerClient.subscribe error:", err);
			}  
		});
	};
	...
	const initHandlers = () => {
		...
		// so hide them.
		$("cq-context").css("height", "97%");
		$("body").css("position", "static");
		$("body").css("overflow", "hidden");
		// $("cq-context").css("top", "0");
		document.querySelector("cq-context").style.top = 0;
		stx.resizeChart();
	};
